...
const NAV_ITEMS = [
  { href: '/dashboard/creator', label: 'Discover', icon: '🏠' },
  { href: '/dashboard/creator/campaigns', label: 'Campaigns', icon: '📋' },
  { href: '/dashboard/creator/inbox', label: 'Inbox', icon: '💬' },
  { href: '/dashboard/creator/earnings', label: 'Earnings', icon: '💰' },
  { href: '/dashboard/creator/profile', label: 'Profile', icon: '👤' },
...
export default async function CreatorDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const profilePhoto = (profile as any)?.profile_photo;
      ...
      >
        ...
        <div style={{ padding: '20px', borderTop: '0.5px solid var(--sk-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            ...
            <div>
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '14px', fontWeight: 600, color: 'var(--sk-text-primary)' }}>{firstName}</div>
              <div style={{ fontFamily: 'Inter, sans-serif', fontSize: '12px', color: 'var(--sk-text-muted)' }}>Creator</div>
            </div>
          </div>
