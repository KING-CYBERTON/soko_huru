...
```
https://soko-huru-jet.vercel.app
```

Every UI task ends with a production screenshot review cycle.
Never mark a task done without completing this loop.

## Post-build Review Loop (every UI task)

```
1. Build the component/section
2. Commit + push to main (triggers Vercel auto-deploy)
3. Wait for Vercel deploy to complete:
   vercel --prod (or push to main triggers it)
4. Screenshot production at desktop (1440px):
   chrome-devtools-mcp → navigate to
   https://soko-huru-jet.vercel.app
   viewport 1440×900 → screenshot
5. Screenshot production at mobile (390px):
   viewport 390×844 → screenshot
6. Inspect computed styles:
   Check .glass-card backdrop-filter resolving
   Check CSS variables resolving to correct hex values
   Check font-family resolving to Plus Jakarta Sans
7. Check browser console for errors:
   Zero console errors required
8. Compare screenshots to Sokohuru design spec
9. Fix any discrepancies → redeploy → re-screenshot
10. Only open PR once screenshots match spec
```
